...
def main():
    ...
    parser.add_argument('--weights', type=str, default='weights/region_model.pth',
    ...
    if args.segment_names is None:
        args.segment_names = ["A-band", "I-band", "intermyofibrillar", "myofibril", "sarco", "mitochondria",
                              "z-disc"]
    ...
    # Create the model
    model = AttUNet(1, len(args.segment_names), filters, final_activation='softmax').to(device)
    if args.weights:
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (images, labels) in enumerate(dataloader):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                ...
                loss = criterion(seg, labels) + 0.1 * torch.mean((images - recon) ** 2)
